...
class SearchService:
    """Сервис для взаимодействия с Elasticsearch: поиск отчетов и документации, построение релевантных ответов."""
    ...
    def get_client(self) -> Optional[Elasticsearch]:
        """
        Создает и возвращает подключение к Elasticsearch, если оно еще не создано.
        
        Возвращает:
            Elasticsearch | None: Клиент или None при ошибке подключения.
        """
    ...
    def search_reports(self, query: str) -> List[str]:
        """
        Ищет релевантные отчеты в Elasticsearch на основе пользовательского запроса.
        
        Аргументы:
            query (str): Пользовательский текст запроса.
        
        Возвращает:
            List[str]: Отсортированный список ID найденных отчетов (в Elasticsearch).
        """
        try:
            es = self.get_client()
            if es is None:
    ...
    def search_documentation(self, query: str) -> List[DocumentResult]:
        """
        Выполняет полнотекстовый поиск в документации, загруженной в Elasticsearch.
        
        Аргументы:
            query (str): Запрос пользователя.
        
        Возвращает:
            List[DocumentResult]: Найденные документы с контентом и релевантностью.
        """
        try:
            es = self.get_client()
            if es is None:
    ...
    def get_ordered_report_options(self, user_query: str) -> List[Dict[str, str]]:
        """
        Возвращает список отчетов, отсортированных по релевантности к пользовательскому запросу.
        
        Аргументы:
            user_query (str): Ввод пользователя.
        
        Возвращает:
            List[Dict[str, str]]: Список словарей с метаданными отчетов (id, title, description).
        """
        try:
    ...
    def _fallback_soft_search(self, query: str) -> List[str]:
        """
        Мягкий fallback-поиск в случае, если основной поиск не дал результатов.
        
        Аргументы:
            query (str): Исходный текст запроса.
        
        Возвращает:
            List[str]: Список ID документов, отобранных по приблизительному совпадению.
        """
        try:
            es = self.get_client()
            if es is None:
