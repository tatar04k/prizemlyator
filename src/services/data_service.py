...
class DataService:
    """Сервис для загрузки, кеширования и валидации производственных данных из разных источников (Elasticsearch, Excel, тестовые данные)."""
    ...
    def load_work_plan_data(self) -> Tuple[Optional[pd.DataFrame], str]:
        """
        Загружает данные по планам работ.
        
        Порядок приоритетов:
        1. Кеш
        2. Elasticsearch (таблица 'oil_event')
        3. Excel-файл
        4. Тестовые данные (если всё остальное недоступно)
        
        Возвращает:
            Tuple[DataFrame | None, str]: Данные и сообщение о результате загрузки.
        """
        ...
        try:
            excel_file = pd.ExcelFile('test_1.xlsx')
    ...
    def load_drilling_data(self) -> Tuple[Optional[pd.DataFrame], str]:     
        """
        Загружает данные по шахматке проб (бурение).
        
        Порядок приоритетов:
        1. Кеш
        2. Elasticsearch (таблица 'chess_rep')
        3. Excel-файл
        
        Возвращает:
            Tuple[DataFrame | None, str]: Загруженные данные и сообщение о результате.
        """
        ...
        st.warning("⚠️ Загружаю данные из локального файла...")
        try:
            df = self.excel_loader.parse_drilling_excel('test_3.xlsx')
    ...
    def load_measurement_data(self) -> Tuple[Optional[pd.DataFrame], str]:
        """
        Загружает данные по замерной добыче нефти.
        
        Порядок приоритетов:
        1. Кеш
        2. Elasticsearch (таблица 'measure_rep')
        3. Excel-файл 
        
        Возвращает:
            Tuple[DataFrame | None, str]: DataFrame и сообщение о результате загрузки.
        """
        ...
        st.warning("⚠️ Загружаю данные из локального файла...")
        try:
            df = self.excel_loader.parse_measurement_excel('test_4.xlsx')
    ...
    def load_gas_utilization_data(self) -> Tuple[Optional[pd.DataFrame], str]:
        """
        Загружает данные по утилизации газа.
        
        Порядок приоритетов:
        1. Кеш
        2. Elasticsearch (таблица 'gaz_rep')
        3. Excel-файл
        ...
        st.warning("⚠️ Загружаю данные из локального файла...")
        try:
            df = self.excel_loader.parse_gas_excel('test_2.xlsx')
    ...
    def get_data_info(self, data_type: str) -> str:
        """
        Возвращает текстовое описание структуры указанного типа данных.
        
        Аргументы:
            data_type (str): Один из 'work_plan', 'drilling', 'measurement', 'gas_utilization'.
        
        Возвращает:
            str: Описание структуры или сообщение об ошибке.
        """
        info_map = {
    ...
    def _get_work_plan_info(self) -> str:
        """
        Возвращает описание структуры соответствующего набора данных в виде строки.
        
        Используется внутренне в `get_data_info`.
        """
        return """
...
"""
    ...
    def _create_test_work_plan_data(self) -> dict:
        """
        Создает и возвращает тестовые данные по планам работы (случай, если ни Excel, ни Elasticsearch недоступны).
        
        Возвращает:
            dict: Словарь, пригодный для создания DataFrame.
        """
        return {
            '№ п/п': range(1, 11),
    ...
    def clear_cache(self):
        """Очищает внутренний кеш загруженных данных."""
        self._cached_data.clear()

    def get_cached_data_types(self) -> list:
        """
        Возвращает список типов данных, которые уже загружены в кеш.
        
        Возвращает:
            list[str]: Названия загруженных наборов данных.
        """
        return list(self._cached_data.keys())

    def validate_data(self, df: pd.DataFrame, data_type: str) -> bool:
        """
        Проверяет, соответствует ли DataFrame ожидаемой структуре по типу данных.
        
        Аргументы:
            df (pd.DataFrame): Проверяемый DataFrame.
            data_type (str): Тип данных ('work_plan', 'drilling', 'measurement', 'gas_utilization').
        
        Возвращает:
            bool: True, если структура данных валидна, иначе False.
        """
