...
def initialize_session_state():
    """Инициализирует ключи состояния Streamlit-сессии при первом запуске."""
...
def strip_latex_from_text(text: str) -> str:
    """Удаляет/преобразует LaTeX-разметку в обычный текст."""
...
def _superscript(digits: str) -> str:
    """Преобразует цифры в строке в верхний индекс (суперскрипт)."""
    return digits.translate(SUPERSCRIPT_MAP)


def render_response_with_latex(response_text: str):
    """Рендерит текст с LaTeX-блоками и обычным содержимым. Поддерживает $$...$$ и align-окружение."""
...
def format_message_content(message_content: str, message_type: str = "text") -> str:
    """Форматирует сообщение по типу: текст, код, ошибка, успех, инфо или предупреждение."""
...
def create_status_indicator(status: str) -> str:
    """Создает HTML-индикатор текущего статуса (с иконкой и цветом)."""
    status_icons = {
        "processing": "🔄",
        "completed": "✅",
        "error": "❌",
...
def format_file_size(size_bytes: int) -> str:
    """Преобразует размер в байтах в человекочитаемый формат: KB, MB и т.д."""
    if size_bytes == 0:
        return "0 B"
...
def create_progress_indicator(current: int, total: int, prefix: str = "") -> str:
    ...
    filled_length = int((current / total) * 20)  
